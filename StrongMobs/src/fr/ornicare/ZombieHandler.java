...
public class ZombieHandler implements Listener {
	...
	public void onCreatureSpawn(final CreatureSpawnEvent e) throws Exception {
		...
		int mobNumber = 0;
		for(World w : Bukkit.getWorlds()) mobNumber+=w.getEntities().size();
		if(mobNumber>2000)
		{
			e.setCancelled(true);
		}
		else
		{
			//Now, check if it's a Zombie for example
			 if(CommandListener.BLOODMOON && e.getEntityType()!=EntityType.ZOMBIE && e.getEntityType()!=EntityType.SKELETON)  {
				/*Creeper zomb = (Creeper)e.getEntity();
				CraftCreeper zombC = (CraftCreeper)zomb;
				EntityCreeper zombMC = zombC.getHandle();

				zombMC.setInvisible(true);
				zombMC.setHealth(0);*/
				 e.setCancelled(true);
		     }
